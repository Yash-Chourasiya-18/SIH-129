...
async def call_department(
    ...
    url = base_url + config["path"].format(citizen_id=citizen_id)
    headers = {
    ...
    mock_getters = {
        "citizen": citizen_registry.get_citizen,
        "education": education_dept.get_education,
        "revenue": revenue_dept.get_revenue,
        "welfare": welfare_dept.get_welfare,
    }
    ...
    try:
        ...
        # Fallback to direct in-memory mock call if loopback HTTP call fails (e.g. cloud host port binding)
        getter = mock_getters.get(dept_key)
        data = getter(citizen_id) if getter else None
        elapsed_ms = (time.perf_counter() - start) * 1000 + simulated_delay
